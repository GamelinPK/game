...
// Helper Functions (Now they take 'room' so they can log details!)
...
const cards = {
    ...
    'shield': { name: 'Iron Wall', icon: '🛡️', desc: 'Gain 10 Armor.', color: '#3742fa', action: (a, t, room) => addArmor(a, 10, room) },
    ...
    'preparation': { name: 'Preparation', icon: '🎒', desc: 'Gain 5 Armor. Draw 2 cards.', color: '#95a5a6', action: (a, t, room) => { addArmor(a, 5, room); drawCards(a, 2, room); } },
    ...
    'panic': { name: 'Panic Button', icon: '🚨', desc: 'If HP < 30, gain 25 Armor. Else, gain 5.', color: '#c0392b', action: (a, t, room) => { if (a.hp < 30) addArmor(a, 25, room); else addArmor(a, 5, room); } },
...
function broadcastState(roomId) {
    ...
    io.to(room.p1.id).emit('game_state', { ...room, me: 'p1', opponent: 'p2', myHand: room.p1.hand, oppHandCount: p2HandCount });
    if(room.p2) {
        io.to(room.p2.id).emit('game_state', { ...room, me: 'p2', opponent: 'p1', myHand: room.p2.hand, oppHandCount: p1HandCount });
    }
}
...
io.on('connection', (socket) => {
    ...
    socket.on('create_room', () => {
        ...
        rooms[roomId] = {
            id: roomId,
            p1: { id: socket.id, name: "Player 1", hp: MAX_HP, armor: 0, poison: 0, regen: 0, hand: getRandomHand(5) },
            p2: null,
            turn: 'p1',
    ...
    socket.on('join_room', (roomId) => {
        ...
        if (rooms[roomId] && !rooms[roomId].p2) {
            rooms[roomId].p2 = { id: socket.id, name: "Player 2", hp: MAX_HP, armor: 0, poison: 0, regen: 0, hand: getRandomHand(5) };
            socket.join(roomId);
    ...
    socket.on('play_card', (data) => {
        ...
        // 2. Execute Card Action (This may draw extra cards)
        ...
        if (!room.gameOver) {
            // 3. End of turn standard draw (replaces the card played)
